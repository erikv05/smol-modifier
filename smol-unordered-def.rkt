...
(define (simple-def? d)
  (type-case Definition d
    [(Defvar x e)
     (simple-expr? e)]
    [(Deffun f as b) #t]))
(define (simple-def/term? t)
  (type-case Term t
    [(definitive d) (simple-def? d)]
    [(expressive e) #f]))
(define (reorder-ts ts)
  (append (filter simple-def/term? ts)
          (filter (not/c simple-def/term?) ts)))
...
(define (evaluate [p : Program])
  (local [(define xs (declared-identifiers p))
    ...
    ((exercute-terms-top-level the-top-level-env) (reorder-ts p))))
