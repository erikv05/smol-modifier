...
(define-type-alias EnvironmentFrame (Hashof Identifier (Optionof Value)))
(define-type-alias Environment (Listof EnvironmentFrame))

(define (load p)
  (some (primitive p)))
(define the-primordial-env
  (map make-hash (make-the-primordial-env load)))
...
(define (env-declare [env : Environment]) : Environment
  (cons (make-hash (list)) env))
(define (env-frame-lookup f x)
  (hash-ref f x))
(define (env-declare! [env : Environment])
  : (Identifier -> Void)
  (lambda (x)
    (unless (hash-has-key? (first env) x)
      (hash-set! (first env) x (none)))))
(define (env-update! [env : Environment])
  : (Identifier Value -> Void)
  (lambda (x [v : Value])
    (hash-set! (first env) x (some v))))
(define (env-lookup [env : Environment] x)
  (let ([v (env-frame-lookup (first env) x)])
...
(define (def-or-set env x mk-v)
  (let* ([_ ((env-declare! env) x)]
         [v (mk-v)]
...
(define (eval-def [env : Environment] d)
  (type-case Definition d
    [(Defvar x e)
     (def-or-set env x (lambda () ((eval-exp env) e)))]
    [(Deffun f xs b)
     (def-or-set env f (lambda () (function xs b env)))]))

(define (eval-body [env : Environment] [xvs : (Listof (Pairof Identifier Value))] b)
  : Value
  (local [(define new-env (env-declare env))]
    (begin
      ; bind arguments
      (for ([xv xvs])
...
(define (evaluate [p : Program])
  (local [(define the-top-level-env (env-declare the-primordial-env))]
