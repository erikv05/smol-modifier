...
(define (env-update! env)
  (lambda (x v)
    (set-box! (env-lookup-ref env x) (copy-value v))))
(define (env-lookup-ref env x)
  ...
  (type-case Definition d
    [(Defvar x e)
     (let ([v ((eval-exp env) e)])
       (let ([_ ((env-update-ref! env) x (box (copy-value v)))])
         (void)))]
    [(Deffun f xs b)
     (let ([_ ((env-update-ref! env) f (box (function xs b env)))])
       (void))]))
...
(define (eval-body env xvs b)
  ...
  (local [(define xs
    ...
    (begin
      ; bind arguments
      (for ([xv xvs])
        ((env-update-ref! new-env) (fst xv) (snd xv)))
