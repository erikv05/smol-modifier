...
(define-type-alias Value (-> PreValue))
(define-type PreValue
  (unit)
  (embedded [c : Constant])
...
(define (as-logical v) : Boolean
  (type-case PreValue (force v)
    [(embedded c)
...
(define (delta [p : PrimitiveOperator] [vs : (Listof Value)])
  (type-case PrimitiveOperator p
    ...
    [(VecNew) (from-vector (list->vector vs))]
    [(VecLen)
...
(define (eval-exp [env : Environment])
  ...
  (lambda ([e : Expression])
    (type-case Expression e
      [(ECon c) (thunk (embedded c))]
      ; [(Var x) (let ([v (env-lookup env x)])
      ;            (thunk (force v)))]
      [(Var x) (thunk (force (env-lookup env x)))]
      [(Lambda xs b) (thunk (function xs b env))]
      [(Let xes b)
       (thunk
        (local [(define (ev-bind xv)
          ...
          (let ([xvs (map ev-bind xes)])
            (force (eval-body env xvs b)))))]
      [(Begin es e)
       (thunk
        (let ([_ (map force (map (eval-exp env) es))])
          (force ((eval-exp env) e))))]
      [(Set! x e)
       (thunk
        (let ([v (force ((eval-exp env) e))])
          (let ([_ ((env-update! env) x (thunk v))])
            (unit))))]
                   ...
                   [(cons eb ebs)
                    (let ([v ((eval-exp env) (fst eb))])
                      (thunk
                       (let ([l (as-logical v)])
                         (if l
                             (force (eval-body env (list) (snd eb)))
                             (force (loop ebs))))))]))]
         (loop ebs))]
      [(App e es)
       (let ([v ((eval-exp env) e)])
            ...
            (type-case PreValue (force v)
              [(function xs b env)
               (if (= (length xs) (length vs))
                   (force (eval-body env (map2 pair xs vs) b))
