#lang plait
#|
Sequences of definitions behave like nested letrec: can refer to itself and previous definitions.
|#
  ...
  (type-case Value v
    [(embedded c)
     (type-case Constant c
       [(logical b) b]
       [else (error 'letrec*-def "expecting a boolean")])]
    [else
     (error 'letrec*-def "expecting a boolean")]))
...
(define (as-numeric v) : Number
  (type-case Value v
    [(embedded c)
     (type-case Constant c
       [(numeric n) n]
       [else (error 'letrec*-def "expecting a number")])]
    [else
     (error 'letrec*-def "expecting a number")]))
...
(define (as-vector v) : (Vectorof Value)
  (type-case Value v
    [(vector v)
     v]
    [else
     (error 'letrec*-def "expecting a vector")]))
(define (from-vector [n : (Vectorof Value)]) : Value
  (vector n))
(define (as-pair [v : Value]) : (Vectorof Value)
  (let ([v (as-vector v)])
    (if (= (vector-length v) 2)
        v
        (error 'letrec*-def "expecting a pair"))))

(define (as-one [vs : (Listof Value)]) : Value
  (cond
    [(= (length vs) 1)
     (first vs)]
    [else
     (error 'letrec*-def "arity-mismatch, expecting one")]))
(define (as-two [vs : (Listof Value)]) : (Value * Value)
  (cond
    [(= (length vs) 2)
     (values (first vs) (first (rest vs)))]
    [else
     (error 'letrec*-def "arity-mismatch, expecting two")]))
(define (as-three [vs : (Listof Value)]) : (Value * Value * Value)
  (cond
    [(= (length vs) 3)
     (values (first vs) (first (rest vs)) (first (rest (rest vs))))]
    [else
     (error 'letrec*-def "arity-mismatch, expecting three")]))
...
(define (delta [p : PrimitiveOperator] [vs : (Listof Value)])
...
(define (env-declare [env : Environment] xs)
  (begin
    (when (has-duplicates? xs)
      (error 'letrec*-def "can't define a variable twice"))
...
(define (env-lookup-location [env : Environment] x)
  : (Boxof (Optionof Value))
  (type-case Environment env
    [empty
     (error 'letrec*-def (string-append "variable undeclared: " (symbol->string x)))]
    [(cons f fs)
     (type-case (Optionof (Boxof (Optionof Value))) (env-frame-lookup f x)
  ...
  (lambda (x [v : Value])
    (let ([loc (env-lookup-location env x)])
      (begin
        (set-box! loc (some v))
        (void)))))
(define (env-lookup [env : Environment] x)
  (let ([v (env-lookup-location env x)])
    (type-case (Optionof Value) (unbox v)
      [(none) (error 'letrec*-def "refer to a variable before assign it a value")]
      [(some v) v])))
...
(define (eval-def [env : Environment] d)
  (type-case Definition d
    [(Defvar x e)
     (let* ([env (extend-env (list) (list x) env)]
            [v ((eval-exp env) e)]
            [_ ((env-update! env) x v)])
         env)]
    [(Deffun f xs b)
     (let* ([env (extend-env (list) (list f) env)]
            [v (function xs b env)]
            [_ ((env-update! env) f v)])
       env)]))

(define (extend-env xvs extra-xs base-env)
  (local {(define vs (map snd xvs))
          (define xs
            (append (map fst xvs) extra-xs))
          (define new-env (env-declare base-env xs))}
    (begin
      ; bind arguments
      (for ([xv xvs])
...
(define (eval-body [env : Environment] [xvs : (Listof (Pairof Identifier Value))] b)
  : Value
  (let [(env (extend-env xvs (list) env))]
    (begin
      ; evaluate starting terms
      (for ([t (fst b)])
        (type-case Term t
          [(definitive d)
           (set! env (eval-def env d))]
          [(expressive e)
           (let ([_ ((eval-exp env) e)])
            (void))]))
                 ...
                 (type-case (Listof (Pairof Expression Body)) ebs
                   [empty
                    (type-case (Optionof Body) ob
                      [(none) (error 'letrec*-def "fall throught cond")]
                      [(some b)
           ...
           (type-case Value v
             [(function xs b env)
              (if (= (length xs) (length vs))
                  ...
                  (error 'letrec*-def "(arity-mismatch (length xs) (length vs))"))]
             [(primitive p)
              (delta p vs)]
             [else
              (error 'letrec*-def "(type-mismatch (TFun) v)")])))])))
...
(define (evaluate [p : Program])
  (local [(define the-top-level-env the-primordial-env)]
