#lang plait
#|
Like referential semantics but shallowly copy values when bind.
|#
...
(define (copy-value v)
  (type-case Value v
    [(vector v) (vector (vector-map (lambda (x) x) v))]
    [else v]))
...
(define (load x) (box (some (primitive x))))
           ...
           (type-case Value v
             [(function xs b env)
              (if (= (length xs) (length vs))
                  (eval-body env (map2 pair xs (map copy-value vs)) b)
