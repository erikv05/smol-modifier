#lang plait
#|
Functions are first-order value -- they can't be stored in vectors not passed around.
|#
...
(define (assert-not-fun [v : Value])
  (type-case Value v
    [(primitive o) (error 'first-order "can't pass functions around")]
    [(function _xs _body _env) (error 'first-order "can't pass functions around")]
    [else v]))
    ...
    [(VecSet)
     (local ((define-values (v1 v2 v3) (as-three vs))
       ...
       (begin
         ...
         (unit)))]
    [(PairNew)
     (local ((define-values (_v1 _v2) (as-two vs)))
...
(define (eval-body [env : Environment] [xvs : (Listof (Pairof Identifier Value))] b)
  ...
  (local [(define vs (map snd xvs))
    ...
    (begin
      ...
      (assert-not-fun ((eval-exp new-env) (snd b))))))
...
(define (eval-exp [env : Environment])
  ...
  (lambda ([e : Expression])
    ...
    (type-case Expression e
      [(ECon c) (embedded c)]
      [(Var x) (assert-not-fun (env-lookup env x))]
      ...
      [(Cond ebs ob)
       (local [(define (loop ebs) : Value
         ...
         (loop ebs))]
      [(App e es)
       (let ([v (env-lookup env (Var-x e))]) ;; the first expression must be a variable
