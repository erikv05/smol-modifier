...
(define (mk-function xs b env)
  (if (empty? xs)
      (thunk-fun (eval-body env (list) b))
      (mk-function xs b env)))
    ...
    [(Defvar x e)
     (let ([v ((eval-exp env) e)])
       (let ([_ ((env-update! env) x v)])
         (void)))]
    [(Deffun f xs b)
     (let ([_ ((env-update! env) f (mk-function xs b env))])
       (void))]))
...
(define (eval-exp env)
  (lambda (e)
    (type-case Expression e
      ...
      [(Lambda xs b) (mk-function xs b env)]
      [(Let xes b)
