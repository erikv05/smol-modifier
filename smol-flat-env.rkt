#lang plait
#|
Like `smol-referential`, but there is only one environment.
|#
...
(define (env-declare [env : Environment] xs)
  (begin
    ...
    (local [(define (allocate x)
              (pair x (box (none))))]
      (begin
        (set-box! env (append (map allocate xs) (unbox env)))
        env))))
...
(define (env-lookup-location [env : Environment] x)
  ...
  (type-case (Optionof (Boxof (Optionof Value))) (env-frame-lookup (unbox env) x)
